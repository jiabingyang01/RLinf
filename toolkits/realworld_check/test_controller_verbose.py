...
def main():
    print("=" * 60)
    print("DEBUG: Starting test_controller (verbose mode)")
    print("=" * 60)
    ...
    print("DEBUG: This will create a Ray worker and initialize FrankaController")
    print("DEBUG: The initialization includes:")
    print("  1. Starting roscore (if not running)")
    print("  2. Launching impedance controller via roslaunch")
    print("  3. Initializing ROS channels")
    print("DEBUG: Please wait, this may take 10-30 seconds...")
    print("-" * 60)

    launch_start = time.time()
    try:
        controller = FrankaController.launch_controller(robot_ip=robot_ip)
        print(f"DEBUG: Controller launched successfully (took {time.time() - launch_start:.1f}s)")
    ...
    try:
        while not controller.is_robot_up().wait()[0]:
            ...
            if elapsed > 30:
                ...
                print("  - Robot is not in FCI (programming) mode")
                print("  - Network connection issues")
                print("  - Robot has errors that need to be cleared")
    except KeyboardInterrupt:
        print("\nDEBUG: Interrupted by user during robot ready check")
        sys.exit(0)
    except Exception as e:
        print(f"ERROR: Failed during robot ready check: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
